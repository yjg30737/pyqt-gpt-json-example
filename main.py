...
from PyQt5.QtWidgets import QMainWindow, QApplication, QVBoxLayout, QWidget, QLabel, QSplitter, QTableWidget, QSizePolicy, QSpacerItem, QHBoxLayout, QPushButton, \
...
class MainWindow(QMainWindow):
    ...
    def __initUi(self):
        ...
        self.__splitter.addWidget(self.__tableWidget)
    ...
    def __run(self):
        ...
        columns = [self.__colListWidget.item(i).text() for i in range(self.__colListWidget.count())]

        self.__wrapper.set_topic(topic)
        self.__wrapper.set_rows(rows)
        self.__wrapper.set_columns(columns)
    ...
    def __generatedFinished(self, result: dict):
        ...
        data = [(i, k, v) for i, (k, v) in enumerate(result.items())]
        self.__tableWidget.setRowCount(len(data[0][2]))

        for i, k, vs in data:
            for j, v in enumerate(vs):
                self.__tableWidget.setItem(j, i, QTableWidgetItem(v))
